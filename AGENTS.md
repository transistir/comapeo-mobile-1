...
capture pipeline. Two documents cover it, and they are plain markdown rather
than tool-specific config so that every agent harness working in this repo
reads the same instructions:

- [`docs/storybook-capture.md`](./docs/storybook-capture.md) — the golden
  ...
  run.
- [`docs/storybook-capture-gate.md`](./docs/storybook-capture-gate.md) — the
...
Claude Code additionally exposes these as the `comapeo-storybook-capture` and
`comapeo-storybook-capture-gate` skills, which are thin pointers to the two
documents above.
